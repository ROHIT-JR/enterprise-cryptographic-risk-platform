import pytest
from lifecycle_engine.models import LifecycleState, GovernanceStatus, TransitionRequest
from lifecycle_engine.engine import LifecycleEngine

def test_default_state_initialization():
    req = TransitionRequest(target_state=LifecycleState.ASSESSED, source="system", is_automated=True)
    # The DB naturally initializes assets as DISCOVERED + ACTIVE, Engine should accept transition
...
def test_invalid_automated_transition():
    req = TransitionRequest(target_state=LifecycleState.MIGRATING, source="system", is_automated=True)
...
def test_manual_transition_authorization():
    req = TransitionRequest(target_state=LifecycleState.MIGRATING, source="user", is_automated=False)
    res = LifecycleEngine.evaluate(LifecycleState.MIGRATION_PLANNED, GovernanceStatus.ACTIVE, req)
...
def test_administrator_force_override():
    req = TransitionRequest(target_state=LifecycleState.RETIRED, source="user", is_automated=False, force_override=True, actor_role="administrator")
...
def test_force_override_denied_for_non_admin():
    req = TransitionRequest(target_state=LifecycleState.RETIRED, source="user", is_automated=False, force_override=True, actor_role="security_analyst")
    res = LifecycleEngine.evaluate(LifecycleState.DISCOVERED, GovernanceStatus.ACTIVE, req)
    assert not res.success
    assert "Only administrators can force override" in res.error

def test_idempotent_repeated_transition():
    req = TransitionRequest(target_state=LifecycleState.ASSESSED, target_governance_status=GovernanceStatus.ACTIVE, source="system", is_automated=True)
...
def test_blocked_governance_status():
    req = TransitionRequest(target_state=LifecycleState.RECOMMENDED, target_governance_status=GovernanceStatus.BLOCKED, source="optimizer", is_automated=True)
...
def test_deferred_status():
    req = TransitionRequest(target_governance_status=GovernanceStatus.DEFERRED, source="user", is_automated=False)
    res = LifecycleEngine.evaluate(LifecycleState.RECOMMENDED, GovernanceStatus.ACTIVE, req)
    assert res.success
...
def test_valid_m4_wave_to_migration_planned():
    req = TransitionRequest(target_state=LifecycleState.MIGRATION_PLANNED, source="optimizer", is_automated=True)
    res = LifecycleEngine.evaluate(LifecycleState.RECOMMENDED, GovernanceStatus.ACTIVE, req)
    assert res.success
