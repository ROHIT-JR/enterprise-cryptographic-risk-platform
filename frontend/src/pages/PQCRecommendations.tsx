          ...
          {cryptographic.map((item, index) => {
            ...
            const card = (
              <Card className="card-hover overflow-hidden">
                <div className="border-b p-5 md:p-6" style={{ borderColor: "var(--border)", background: "var(--bg-hover)" }}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: "var(--accent)" }}>
                        Wave {item.wave} · {item.complexity} complexity
                      </p>
                      <h2 className="mt-1 text-base font-bold" style={{ color: "var(--text-primary)" }}>{item.asset_name}</h2>
                    </div>
                ...
                <div className="p-5 md:p-6">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2.5">
                    {Object.entries(metrics).slice(0, 4).map(([key, value]) => (
                      <div key={key} className="rounded border p-2.5" style={{ borderColor: "var(--border)", background: "var(--bg-hover)" }}>
                        <p className="font-mono text-[9px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>
                          {key.replaceAll("_", " ")}
                        </p>
                        <p className="mt-0.5 font-mono text-xs font-bold capitalize" style={{ color: "var(--text-primary)" }}>{value}</p>
                      </div>
                    ))}
                  </div>
                  {item.recommendation.hybrid_strategy && (
                    <div
                      className="mt-4 flex items-start gap-2 rounded border p-3 text-xs leading-relaxed font-medium"
                      style={{ borderColor: "var(--accent)", background: "var(--accent-soft)", color: "var(--text-primary)" }}
                    >
                      <Gauge className="mt-0.5 h-4 w-4 shrink-0" style={{ color: "var(--accent)" }} />
...
function State({ label, value, icon: Icon }: { label: string; value: string; icon: LucideIcon }) {
  return (
    <div className="rounded border p-3 shadow-xs" style={{ borderColor: "var(--border)", background: "var(--bg-card)" }}>
      <Icon className="h-4 w-4" style={{ color: "var(--accent)" }} strokeWidth={1.75} />
      <p className="mt-2 font-mono text-[9px] font-bold uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>{label}</p>
      <p className="mt-0.5 font-mono text-xs font-bold" style={{ color: "var(--text-primary)" }}>{value}</p>
    </div>
  );
}
