...
export function MoscaTimeline({ dataLifetime, migrationTime, quantumArrivalYear, currentYear }: MoscaTimelineProps) {
  ...
  return (
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-full w-full" role="img" aria-label="Mosca inequality timeline">
      ...
      <rect x={PAD_LEFT} y={20} width={Math.max(zoneX - PAD_LEFT, 0)} height={AXIS_Y - 20} fill="rgba(52,211,153,0.06)" />
      <rect x={zoneX} y={20} width={Math.max(WIDTH - PAD_RIGHT - zoneX, 0)} height={AXIS_Y - 20} fill="rgba(251,113,133,0.08)" />

      {/* axis */}
      <line x1={PAD_LEFT} y1={AXIS_Y} x2={WIDTH - PAD_RIGHT} y2={AXIS_Y} stroke="rgba(148,163,184,0.25)" />
      {ticks.map((tick) => (
        <g key={tick.year}>
          <line x1={tick.x} y1={AXIS_Y} x2={tick.x} y2={AXIS_Y + 6} stroke="rgba(148,163,184,0.35)" />
      ...
      <text x={PAD_LEFT} y={38} fontSize="11" fill="currentColor" className="font-medium text-brand-300">
        X · Data lifetime ({dataLifetime}y)
      </text>

      {/* Y bar: migration time */}
      <rect x={PAD_LEFT} y={76} width={Math.max(yBarEnd - PAD_LEFT, 2)} height={16} rx={8} fill="#f59e0b" fillOpacity={0.75} />
      <text x={PAD_LEFT} y={70} fontSize="11" fill="currentColor" className="font-medium text-amber-400">
        Y · Migration time ({migrationTime}y)
      </text>

      {/* combined X+Y marker */}
      <line x1={lhsX} y1={30} x2={lhsX} y2={AXIS_Y} stroke="#e2e8f0" strokeDasharray="4 3" strokeWidth={1.5} />
      <text x={lhsX} y={22} textAnchor="middle" fontSize="10" fill="currentColor" className="text-slate-300">
        Protected until Y{Math.round(lhs)}
      </text>

      {/* Z marker: quantum arrival */}
      <line x1={zoneX} y1={16} x2={zoneX} y2={AXIS_Y} stroke="#fb7185" strokeWidth={2} />
      <text x={zoneX} y={108} textAnchor="middle" fontSize="11" fontWeight={700} fill="#fb7185">
        Q-Day
      </text>
      <text x={zoneX} y={122} textAnchor="middle" fontSize="9" fill="#fb7185">
        {quantumArrivalYear}
      </text>

      {/* verdict banner */}
      <text x={PAD_LEFT} y={HEIGHT - 6} fontSize="12" fontWeight={700} fill={verdict === "critical" ? "#fb7185" : "#34d399"}>
        {verdict === "critical"
