...
export function MoscaSection() {
  ...
  const statusCopy: Record<typeof data.organization_status, { label: string; color: string }> = {
    ...
    plan: { label: "PLAN REQUIRED", color: "text-amber-300" },
    safe: { label: "SAFE", color: "text-emerald-300" },
  };
  const status = statusCopy[data.organization_status];

  return (
    <Card className="overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-white/[0.06] px-5 py-5 md:flex-row md:items-center md:justify-between md:px-6">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-brand-300">The core differentiator</p>
          <h2 className="mt-1 text-lg font-semibold text-white">Mosca inequality: X + Y &gt; Z</h2>
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 self-start rounded-xl border px-3.5 py-2 text-xs font-semibold transition ${
            ceoMode ? "border-brand-300/40 bg-brand-400/10 text-brand-200" : "border-white/[0.08] bg-white/[0.02] text-slate-400"
          }`}
        >
      ...
      <div className="mx-5 mb-5 rounded-xl border border-white/[0.07] bg-black/15 p-4 md:mx-6 md:mb-6">
        ...
        {ceoMode ? (
          ...
          <div className="mt-3 space-y-1.5 text-sm text-slate-400">
            ...
            {data.most_urgent_asset && <p className="text-rose-300">Most urgent: {data.most_urgent_asset}</p>}
          </div>
        )}
      </div>

      <div className="border-t border-white/[0.06] px-5 py-4 md:px-6">
        <p className="text-sm font-semibold text-white">Per-asset Mosca breakdown</p>
